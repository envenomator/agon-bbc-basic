...
OSKEY:			CALL	READKEY			 ; Read the keyboard 
			...
			JR	OSKEY 			 ; And loop
;
1:			LD	HL, KEYDOWN		 ; We have a key, so 
			LD	(HL), 0			 ; clear the keydown flag
			CP	0x1B			 ; If we are not pressing ESC, 
...
;		
COMDS:  		.byte	'A','S','M'+0x80		 ; ASM
			.d24	STAR_ASM
			.byte	'B','Y','E'+0x80		 ; BYE
			.d24	STAR_BYE
			.byte	'E','D','I','T'+0x80		 ; EDIT
			.d24	STAR_EDIT
			.byte	'F','X'+0x80		 ; FX
			.d24	STAR_FX
			.byte	'V','E','R','S','I','O','N'+0x80	 ; VERSION
			.d24	STAR_VERSION
			.byte	0xFF
...
STAR_VERSION:		CALL    TELL			 ; Output the welcome message
			.asciz    	"BBC BASIC (Agon ADL) Alpha Version 0.01\n\r"
			RET
...
;  H: Second parameter
;
OSBYTE:			CP	0x0B			 ; *FX 11, n: Keyboard auto-repeat delay
			JR	Z, OSBYTE_0B
			CP	0x0C			 ; *FX 12, n: Keyboard auto-repeat rate
			JR	Z, OSBYTE_0C
			CP	0x13			 ; *FX 19: Wait for vblank
			JR	Z, OSBYTE_13		
			CP	0x76			 ; *FX 118, n: Set keyboard LED
			JP	Z, OSBYTE_76
			CP	0xA0
			JP	Z, OSBYTE_A0		
...
;
1:			CALL	OSBGET			 ; Read the byte into A
...
;
OSLOAD_TXT4:		CP	0x20			 ; Skip if not an ASCII character
			JR	C, 1f
...
1:			POP	HL			 ; Restore the filename pointer
			RET
...
;
1:			LD	A, (DE)			 ; Skip to the end of the entry in the lookup
			INC	DE
			OR	A
			JR	NZ, 1b
...
;
EXT_LOOKUP:		.byte	'.','B','B','C', 0, 0		 ; First entry is the default extension
			.byte	'.','T','X','T', 0, 1
			.byte	'.','A','S','C', 0, 1
			.byte	'.','B','A','S', 0, 1
			.byte	0			 ; End of table
